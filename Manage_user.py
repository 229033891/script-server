...
def modify_user_file(username, htpasswd_file, new_password=None, delete=False):
...
def change_user_password(username, htpasswd_file, manual=True):
    ...
    if old_password == "@" or not verify_user_password(username, old_password, htpasswd_file):
        print("\033[91m旧密码错误或取消操作。\033[0m")
        return
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            username = input_with_timeout("请输入用户名: ", 60)
            if username and verify_user_password(username, "", htpasswd_file):
